...
public abstract class Ship implements Serializable {
	...
	public void addWeapon(Weapon newWeapon) {
		if (quality >= newWeapon.getMinQuality() && weaponSlot
                        < weaponSlots.length) {
	...
	/**
	 * Resets the ship's current travelable range to the maximum travelable
         * range.
	 */
	public void refuel() {
		currRange = maxRange;
	 ...
	 * @param opponent The opposing ship
         * @return Total possible damage ship can deal
	 */
	public int dealDamage(Ship opponent) {
            int damageTaken = 0;
            if (opponent == null || weaponSlots.length == 0 || weaponSlots[0]
                    == null) {
                return damageTaken;
            } else {
                for (int i = 0; i < weaponSlot; i++) {
                    if (weaponSlots[i].getAmmo() > 0) {
                        ...
                        opponent.takeDamage((int) (weaponSlots[i].getDamage()
                                * Math.random()));
	...
	/**
	 * Determines if the ship has enough room to hold a particular amount
         * of cargo.
	...
	/**
	 * Determines if the ship has enough cargo to sell in the specified
         * amounts.
	 * @param name The name of the cargo being sold
	 * @param amount The amount the player wants to sell
	 * @return A boolean determining whether or not the player can sell the
         * specified cargo in the specified amounts
	 */
	public boolean canSell(String name, int amount) {
	    return cargo.containsKey(name.toLowerCase())
                    && cargo.get(name.toLowerCase()) >= amount;
	}
